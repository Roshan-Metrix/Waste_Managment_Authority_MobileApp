...
const [storeInfo, setStoreInfo] = useState({ id: null, name: "Loading Store..." });
...
try {
 ...
 const response = await api.get(`/manager/transaction/store-total-transactions/${getStoreId}`);
 ...
 Alert.alert("API Error", response.data?.message || "Failed to fetch transactions.");
 }
} catch (error) {
...
<View style={styles.container}>
 ...
 {/* Refresh Button */}
 <TouchableOpacity onPress={fetchTransactions} style={styles.headerButton}>
 ...
 <Text style={styles.emptyText}>No transactions recorded for this store.</Text>
 ) : (
 ...
 >
  ...
  <Text style={styles.detailText}>
  Date: <Text style={{ fontWeight: '600' }}>{formatTimestamp(txn.createdAt)}</Text>
  </Text>
  <Text style={styles.detailText}>
  Total Items: <Text style={{ fontWeight: '600' }}>{txn.items.length}</Text>
  </Text>
  </View>
 </TouchableOpacity>
 ))
